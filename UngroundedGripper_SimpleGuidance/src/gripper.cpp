...
gripper::gripper(void){	//:  pThumb(fingers::thumb), pIndex(fingers::index)
	...
	// kinematic variables
	m_t = 0;
	m_thZero = { 0.8796 , 2.2620, 0.8796 , 2.2620, 0 }; //{ PI / 2.0, PI / 2.0, PI / 2.0, PI / 2.0, 0.0 };
	m_clk = new cPrecisionClock();
	m_th = m_thZero;		//{ PI / 2.0, PI / 2.0, PI / 2.0, PI / 2.0, 0.0 };
	m_thDes = m_thZero;
...
bool gripper::calibrate(void) {
	...
	cout << "Move pantographs to center of workspace. Hold gripper closed." << endl;
	Sleep(1000);
...
void gripper::setForcesAndTorques(cVector3d a_force, cVector3d a_torque, double a_gripForce, cVector3d a_thumbForce, cVector3d a_fingerForce) {
	...
	if (0) {
		...
		cout << "Index Actual Angles: " << cRadToDeg(pIndex.m_th(0)) << ", " << cRadToDeg(pIndex.m_th(1)) << "     ";
		cout << "Thumb Actual Angles: " << cRadToDeg(pThumb.m_th(2)) << ", " << cRadToDeg(pThumb.m_th(3)) << "     ";
