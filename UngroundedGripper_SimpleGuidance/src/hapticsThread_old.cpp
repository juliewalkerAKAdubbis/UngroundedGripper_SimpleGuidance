...
void hapticsThread::updateHaptics(void)
{
	...
	while (simulationRunning)
	{
		...
		m_worldLock.acquire();
		position = tool->getGlobalPos();
		rotation = tool->getLocalRot();
		m_worldLock.release();
		...
		cout << "H Thread test val: " << testHapticsThreadConnection() << endl;
...
double hapticsThread::testHapticsThreadConnection(void) {
	return testVal;
}