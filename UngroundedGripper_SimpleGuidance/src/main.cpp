...
void experimentThreadHandler(void);
...
gripper* G; 						// create a gripper instance

magTrackerThread MT(0);
cThread* magTracker_thread;			// magnetic tracker

experiment EXP;
cThread* experiment_thread;			// experiment handling
...
int main(int argc, char* argv[])
{
	...
	G = new gripper();			// create a gripper instance
	if (!G->connect()) {
...
void keyCallback(GLFWwindow* a_window, int a_key, int a_scancode, int a_action, int a_mods)
{
	...
	else if (a_key == GLFW_KEY_M)		//down
	{
		...
		double s = 0.0;
		for (int t = 0; t < 10; t++) {
			s += defaultStretch*(t / 10);
			H.m_fingerForce.set( 0.0, 0.0, -s);
			H.m_thumbForce.set( 0.0, 0.0, -s);
			Sleep(50);
		}
	}
	...
	else if (a_key == GLFW_KEY_L)		//forward
	{
		...
		double s = 0.0;
		for (int t = 0; t < 10; t++) {
			s = s + defaultStretch*(t / 10);
			H.m_fingerForce.set(-s, 0.0, 0.0);
			H.m_thumbForce.set(-s, 0.0, 0.0);
			Sleep(100);
			cout << t << "...";
		}
		cout << endl;
	...
	else if (a_key == GLFW_KEY_P)		//twist left
	{
		cout << "TWIST LEFT" << endl;
		double s = 0.0;
		for (int t = 0; t < 10; t++) {
			s = s + defaultStretch*(t / 10);
			H.m_fingerForce.set(-s, 0.0, 0.0);
	...
	else if (a_key == GLFW_KEY_H)		//twist right
	{
		cout << "TWIST RIGHT" << endl;
		double s = 0.0;
		for (int t = 0; t < 10; t++) {
			s = s + defaultStretch*(t / 10);
			H.m_fingerForce.set(s, 0.0, 0.0);
